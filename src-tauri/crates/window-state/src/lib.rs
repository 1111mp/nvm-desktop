    ...
    fn restore_state(&self, flags: StateFlags) -> tauri::Result<()> {
            ...
            .filter(|state| state != &&WindowState::default())
        {
            ...
            if flags.contains(StateFlags::POSITION) {
                ...
                {
                    let scale_factor = self.scale_factor()?;
                    let pos = self.outer_position()?.to_logical(scale_factor);
                    metadata.x = pos.x;
                    metadata.y = pos.y;
                }
            }
...
trait MonitorExt {
    ...
    #[cfg(target_os = "macos")]
    fn intersects(&self, position: LogicalPosition<i32>, size: LogicalSize<u32>) -> bool;
}
...
impl MonitorExt for Monitor {
    ...
    fn intersects(&self, position: LogicalPosition<i32>, size: LogicalSize<u32>) -> bool {
        let scale_factor = self.scale_factor();
        let LogicalPosition { x, y } = self.position().to_logical::<i32>(scale_factor);
        let LogicalSize { width, height } = self.size().to_logical::<u32>(scale_factor);
